...
* [x] 대분류/중분류/소분류 간 양방향 매핑 함수 제공
* [x] 4개 운동 목표 및 3개 환경별로 최소 5개 이상의 대체 후보 운동 확보
...
4. **추천 엔진 단위 테스트(Unit Tests) 작성 (`tests/engine.test.ts`)**
   - 무릎 회피 하체 운동 추천 테스트
   - 런닝 회피 유산소 운동 추천 테스트
   - 홈트 환경 필터링 테스트
   - 조건 불일치 시 0개 반환 테스트
...
#### ✅ 완료 기준 (Acceptance Criteria)
* [x] 입력값 4개 전달 시 5초 이내에 1~3위 결과 리스트 반환
* [x] 필터링된 결과에 회피 부위/싫은 운동이 100% 미포함됨을 단위 테스트로 검증
...
#### ✅ 완료 기준 (Acceptance Criteria)
* [x] 단일 화면 내에 4개 영역(①~④)이 일목요연하게 배치
* [x] 데스크톱 및 모바일 뷰포트에서 스크롤 깨짐 없이 동작
...
#### ✅ 완료 기준 (Acceptance Criteria)
* [x] 6가지 예외 상황이 화면에서 정확한 문구와 함께 재현 및 복구 가능
...
2. **Definition of Done (DoD) 체크리스트 확인**
   - [x] 단일 화면에서 입력(4개 항목) → 추천받기 → 결과(1~3위) 흐름이 새로고침 없이 동작하는가?
   - [x] 5-1~5-6 예외 상황 6가지가 모두 화면에서 재현 및 확인 가능한가?
   - [x] 로그인/결제/DB 저장 기능이 코드 어디에도 존재하지 않는가?
   - [x] 결과는 순위 + 운동명 + 세트/횟수 + 주의사항 텍스트로 구성되어 출력되는가?
   - [x] "다시 추천받기"로 입력값 유지한 채 재요청이 가능한가?
   - [x] 전체 기능이 5시간 작업 범위 내에서 구현 완료되었는가?
